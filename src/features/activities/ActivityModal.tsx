...
import type { ActivityCategory, ActivityLog, ActivityConfig } from '@/types';
...
export function ActivityModal({ isOpen, onClose, category, editingActivity, allConfigs }: ActivityModalProps) {
  ...
  // Usar allConfigs se fornecido, senão fallback para ACTIVITY_CONFIGS
  const configs = allConfigs || ACTIVITY_CONFIGS;
  ...
  useEffect(() => {
    if (editingActivity) {
      ...
      if (!quickDurations.includes(editingActivity.duration)) {
        setCustomDuration(editingActivity.duration.toString());
      }
      setNotes(editingActivity.notes || '');
      setDifficulty(editingActivity.difficulty || 'medium');
      setMood(editingActivity.mood || 3);
    } else {
  ...
  // Success state (apenas para novas atividades)
  if (result) {
    return (
      <>
  ...
  return (
    <Modal
      ...
      onClose={handleClose}
      title={config.name}
      subtitle={isEditing ? 'Editar atividade' : 'Registrar atividade'}
      size="md"
    >
      <div className="space-y-6">
        ...
        {/* Preview (apenas para novas atividades) */}
        {!isEditing && (
